const renderNoProjectsMessage = (container) => {
    const message = document.createElement('p');
    message.classList.add('no-projects-message');
    message.textContent = 'There are no items that satisfy the filter';
    container.innerHTML = '';
...
const dataRender = (data, container) => {
    ...
    if (data.length === 0) {
        renderNoProjectsMessage(container);
        return;
    }
